...
def train_and_evaluate(net, dataloaders, config, device, lin_cls=False):
    ...
    # train_step and evaluate functions remain the same...
    ...
    def evaluate(net, evalloader, loss_fn, config):
        ...
        eval_metrics = get_classification_metrics(predicted=predicted_classes, labels=target_classes, n_classes=num_classes, unk_masks=None)
        ...
        print("-----------------------------------------------------------------------------------------------------------------------------------------------------------------")
        return (un_labels, {"macro": {"Loss": losses.mean(), "Accuracy": macro_acc, "Precision": macro_precision, "Recall": macro_recall, "F1": macro_F1, "IOU": macro_IOU},
                 "micro": {"Loss": losses.mean(), "Accuracy": micro_acc, "Precision": micro_precision, "Recall": micro_recall, "F1": micro_F1, "IOU": micro_IOU},
                 "class": {"Loss": class_loss, "Accuracy": class_acc, "Precision": class_precision, "Recall": class_recall, "F1": class_F1, "IOU": class_IOU}})
    ...
    loss_fn = {'all': get_loss(config, device, reduction=None), 'mean': get_loss(config, device, reduction="mean")}
    trainable_params = get_net_trainable_params(net)
    optimizer = optim.AdamW(trainable_params, lr=float(config['SOLVER']['lr_base']), weight_decay=get_params_values(config['SOLVER'], "weight_decay", 0))
    ...
    for epoch in range(start_epoch, start_epoch + num_epochs):
        for step, sample in enumerate(dataloaders['train']):
            ...
            # Train step, printing, step-based saving, evaluation...
            # This inner loop logic remains the same.
            logits, ground_truth, loss = train_step(net, sample, loss_fn, optimizer, device, loss_input_fn=loss_input_fn)
            if len(ground_truth) == 2: labels, unk_masks = ground_truth
            else: labels, unk_masks = ground_truth, None
            ...
            if abs_step > 0 and abs_step % save_steps == 0:
                checkpoint_data = {'epoch': epoch, 'abs_step': abs_step, 'model_state_dict': net.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'scheduler_state_dict': scheduler.state_dict(), 'best_iou': BEST_IOU}
        ...
        # <<< NEW BLOCK: EPOCH-BASED SAVING >>>
        # This code runs once at the end of each epoch.
        if epoch % 10 == 0 and epoch > 0: # Saves every 10 epochs (10, 20, 30...)
            checkpoint_data = {
                'epoch': epoch,
                'abs_step': abs_step,
            ...
            save_filename = f"{save_path}/checkpoint_epoch_{epoch}.pth"
            torch.save(checkpoint_data, save_filename)
            print(f"Saved epoch-based checkpoint to {save_filename}")
        # <<< END OF NEW BLOCK >>>
